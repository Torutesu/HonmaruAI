...
export const CardThread: React.FC<Props> = ({ httpBase, orgId, sessionToken, cardId, version, alwaysOpen }) => {
  const t = useT()
            ...
            <li key={ev.id} className={`thread-row t-${ev.type}`}>
              ...
              <span className="thread-body">
                <span className="thread-head">
                  {t(eventWord(ev))}
                  {ev.actorUserId && <span className="thread-who"> · {displayName(ev.actorUserId)}</span>}
                </span>
                {ev.note && <span className="thread-note">“{ev.note}”</span>}
              </span>
            </li>
          ))}
        </ol>
