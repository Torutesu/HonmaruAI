...
function readBody(req, { maxBytes = 1024 * 1024 } = {}) {
  return new Promise((resolve, reject) => {
    const tooLarge = () => Object.assign(new Error(`Request body exceeds ${maxBytes} bytes`), { statusCode: 413 });

    // Fast path: a well-formed Content-Length lets us reject before reading
    // any body bytes at all.
    const contentLength = Number(req.headers["content-length"]);
    if (Number.isFinite(contentLength) && contentLength > maxBytes) {
      req.resume(); // drain so the socket isn't left stalled mid-request
      reject(tooLarge());
      return;
    }
    ...
    req.on("data", (chunk) => {
      ...
      size += chunk.length;
      if (size > maxBytes) {
        // Reject but keep consuming (not req.destroy()) — killing the
        // socket here means the response below never reaches the client,
        // which surfaces as a raw connection-reset instead of a clean 413.
        rejected = true;
        reject(tooLarge());
        return;
      }
