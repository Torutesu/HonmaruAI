...
- **Signed iPhone 17 Pro / iOS 26.4: PASS.** The final full run passed all 63 unit/integration tests (including the opt-in live native lifecycle) and both UI tests with zero failures. This includes actual password input, workspace join and relaunch restoration, plus visible text composition, explicit no-AI drafting, editable review and Plan legal links. Result: `/private/tmp/honmaru-integration-iphone-signed-final.xcresult`.
- Remote CI status is available on PR #43. The initial Node 20 E2E failure is superseded by the Node 22 rerun; do not infer all checks passed until GitHub reports success for the current commit.
