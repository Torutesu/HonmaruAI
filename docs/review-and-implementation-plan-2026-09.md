...
| 組織グラフ（役割・責任範囲・承認権限）に基づく配信 | ✅ 実装済（2026-09-04） | `org_profiles` で本人が役割・責任範囲・上長を記述。`manages` エッジが生成され、エスカレーション分岐が生きた。プロンプトは責任範囲を読む |
| 受信側 AI が受信者の役割・状況に合わせて変換 | ✅ 実装済（2026-09-04） | `render.js` + `relay.renderForRecipient`。カード着信後に受信者の役割・責任範囲・自身のコンテキスト・言語で書き直す。受信者の枠で課金、失敗時は送信者版のまま |
| 優先度変更 / AI への追加指示 | ✅ 実装済（2026-09-04） | カード詳細から優先度を変更でき、`POST /ai/refine` で自分の AI にカードの書き直しを頼める |
...
**1-9 GitHub 同期のサーバー移管 — 見送り（2026-09-04 判断）**

- 当初の推奨はサーバー移管だったが、P0-9 で「決定を記録してから GitHub に送る」順序に直した時点で、緊急性のあった失敗（オフラインで承認が消える）は解消済み。残るのは「決定者がアプリを二度と開かないと Issue が作られない」ケースのみ
- サーバー移管を正しくやるには、DO のアラームか `github_sync_queue` テーブル + cron による**永続的なリトライ機構**が要る。それ抜きで移すと、失敗した同期はクライアントの 30 秒リトライすら失って完全に消える（今より悪い）
- したがって 1-4（同期の範囲拡大）はクライアント側で実装した。サーバー移管はリトライ機構とセットで Phase 3 以降の判断に送る
...
**2-5 ゲストモードの整理 — B を採用（2026-09-04 実装済）**

- 当初の推奨は A（廃止）だったが、P0-7 で `/ai/route` が空の組織を 400 で拒否するようになった時点で、B（一人組織として成立させる）が小さな差分で届く範囲に入った
- 実装: 受信者 = 自分固定、リレー接続なし（`enterLocalOnlyMode`）、アウトボックスに積まない、`/ai/route` を叩かず OfflineRouter で下書き、空状態とローカルモードのチップを表示。ゲストからサインインへ移る際にローカルのカードとキューを破棄
- 結果として「指示 → カード → 判断」のコアループがサインイン前に体験でき、App Store 審査でも同じ経路が使える
