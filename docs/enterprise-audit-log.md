...
| **ドメイン参加・招待制限** | 社外ドメインの招待禁止、承認制 | ドメイン確認 | 2 |
| **データの書き出し（管理者）** | ワークスペース全体のコンプライアンス書き出し | 監査ログ（書き出し自体を記録） | 3 |
| **リーガルホールド** | 指定した人・チャネルのメッセージと監査ログを、保持期間に関係なく保全する | 保持期間ポリシー | 3 |
| **保持期間ポリシー** | メッセージ、ファイル、文字起こしの自動削除期間（チャネルの種類別） | — | 3 |
| **IP 許可リスト** | 管理 API と、必要ならアプリ全体への接続元制限 | — | 3 |
| **DLP** | 送信前の機密パターン検知（マイナンバー、カード番号など）と、警告またはブロック。**実装済み**（`worker/src/dlp.js`、Studio の「データルール」）。メッセージの投稿・編集・日報の投稿で読み、添付ファイル（テキスト、Word・Excel・PowerPoint の本文）も読む。監査ログにはルール名と該当ファイルの数だけを残す | — | 4 |
