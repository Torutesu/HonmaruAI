...
**ツール → GitHub → GitHub で接続** を押すと、Gmail や Slack と同じように
認証ページに遷移して GitHub が承認を求め、戻ってくると自分の GitHub の
リポジトリ一覧から 1 つ選ぶだけです（トークンの貼り付けは不要。Composio が
ホストする OAuth で、GitHub 用の auth config は Worker が初回に自動で作って
`kv` テーブルに保存します。`CONNECTOR_AUTH_GITHUB` を設定すればそれが優先）。
トークンを貼る方法も「代わりにトークンを使う」から引き続き使えます。
