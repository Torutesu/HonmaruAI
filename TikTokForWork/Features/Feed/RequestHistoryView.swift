            ...
            List {
                ...
                ForEach(cards) { card in
                    NavigationLink { RequestDetailView(cardID: card.id, service: service) } label: {
                        RequestListRow(card: card, person: queue == .sent ? (card.recipientName ?? name(card.recipientUserID)) : (card.requestedBy?.name ?? name(card.senderUserID)), isSent: queue == .sent, awaitingDelivery: service.awaitingDeliveryIDs.contains(card.id))
                            .padding(.vertical, 8)
                    }
                }
            }.listStyle(.plain)
