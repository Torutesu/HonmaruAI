                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 26) {
                        ...
                        if card.isPending && card.senderUserID == appState.currentUser?.id && card.recipientUserID != appState.currentUser?.id {
                            Label(String(localized: "Waiting for \(card.recipientName ?? name(card.recipientUserID))"), systemImage: "clock").font(.subheadline).foregroundStyle(Theme.Colors.textSecondary)
                            if !appState.isGuest {
                                Button(nudged ? String(localized: "Asked") : String(localized: "Nudge")) {
                                    Task {
                                        await appState.webSocketService.nudge(cardID: card.id)
                                        nudged = true
                                    }
                                }.disabled(nudged || !canAct || service.awaitingDeliveryIDs.contains(card.id))
                            }
                        }
                    }.padding(24)
                }
