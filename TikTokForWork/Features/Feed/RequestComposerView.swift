...
struct RequestComposerView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(spacing: 9) {
                    ...
                    PrimaryButton(title: model.isReviewing ? String(localized: "Send request") : String(localized: "Review request"), enabled: model.isReviewing ? canSend : model.canReview) {
                        ...
                        Task {
                            ...
                            else if appState.isGuest { await model.prepare(appState: appState) }
                            else { confirmAI = true }
                        }
                    }
                    .accessibilityIdentifier("request.primary")
                    if !model.isReviewing { Text("Your team receives the card only after you tap Send request.").font(.caption).foregroundStyle(Theme.Colors.textSecondary) }
                }
            ...
            .alert("Use AI to prepare this draft?", isPresented: $confirmAI) {
                Button("Use AI") { Task { await model.prepare(appState: appState, allowAI: true) } }
                Button("Prepare without AI") { Task { await model.prepare(appState: appState) } }
                Button("Cancel", role: .cancel) { }
            } message: {
                Text("Your text, including any transcript, and relevant team context will be sent to our AI provider, OpenAI. Microphone audio is not sent. You can prepare a draft without AI instead.")
            }
    ...
    private var recipientPicker: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            .accessibilityIdentifier("request.recipient")
