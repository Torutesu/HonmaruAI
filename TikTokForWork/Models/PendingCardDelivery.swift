...
struct PendingCardDelivery: Codable {
    ...
    func matches(_ actual: DecisionCard) -> Bool {
        let sameRecipient = actual.recipientUserID == card.recipientUserID
            || actual.recipientMemberRef.map { "member:\($0)" == card.recipientUserID } == true
        guard actual.id == card.id, sameRecipient,
