...
final class RequestComposerTests: XCTestCase {
    @MainActor
    func testPreparingWithoutExplicitAIConsentUsesManualDraft() async {
        let app = AppState(startServices: false)
        app.currentUser = User(id: "alice", name: "Alice", role: "Member", teamID: "team", githubUsername: nil)
        let composer = FeedViewModel()
        composer.sourceText = "Please check the proposal"
        await composer.prepare(appState: app)
        XCTAssertTrue(composer.isReviewing)
        XCTAssertEqual(composer.preparationNote, String(localized: "Manual draft · no text was sent to AI"))
        XCTAssertFalse(composer.title.isEmpty)
    }
