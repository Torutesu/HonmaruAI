...
final class PendingDeliveryTests: XCTestCase {
    ...
    func testStaleSnapshotAndUnrelatedEnrichmentCannotConfirmADecision() {
        ...
        socket.onEvent?(.snapshot(cardsByUser: ["alice": [original]], onlineUserIds: []))
    ...
    func testOpaqueRecipientIsConfirmedOnlyByMatchingServerEvidence() {
        let outbound = DecisionCard(id: "ref-card", recipientUserID: "member:opaque", senderUserID: "alice",
                                    type: .approval, title: "Review", summary: "Summary", context: "",
                                    status: .pending, priority: .medium, createdAt: .now)
        let expected = PendingCardDelivery(card: outbound, operation: .create)
        var echo = DecisionCard(id: "ref-card", recipientUserID: "u:bob@example.com", senderUserID: "alice",
                                type: .approval, title: "Review", summary: "Summary", context: "",
                                status: .pending, priority: .medium, createdAt: .now)
        XCTAssertFalse(expected.matches(echo))
        echo.recipientMemberRef = "different"
        XCTAssertFalse(expected.matches(echo))
        echo.recipientMemberRef = "opaque"
        XCTAssertTrue(expected.matches(echo))
        echo.summary = "Unrelated update"
        XCTAssertFalse(expected.matches(echo))
    }
